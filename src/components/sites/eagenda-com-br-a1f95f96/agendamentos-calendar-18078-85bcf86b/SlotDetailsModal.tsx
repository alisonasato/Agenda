...
const ACTIONS: Record<string, { label: string; action: CalendarAction; icon: React.ReactNode }[]> = {
  PENDING: [
    { label: "Confirmar", action: "accept", icon: <CheckReadIcon className="w-4 h-4" /> },
    { label: "Recusar", action: "reject", icon: <CloseCircleIcon className="w-4 h-4" /> },
  ],
  CONFIRMED: [
    { label: "Registrar Chegada", action: "attend", icon: <CheckReadIcon className="w-4 h-4" /> },
    { label: "Não Compareceu", action: "no_show", icon: <DangerCircleIcon className="w-4 h-4" /> },
    { label: "Cancelar Agendamento", action: "cancel", icon: <TrashIcon className="w-4 h-4" /> },
  ],
  ATTENDED: [],
  NO_SHOW: [],
  CANCELED: [],
    ...
    <>
      ...
      >
        ...
        <div className="flex flex-wrap items-center gap-2 mb-6 pb-4 border-b border-gray-200">
          {chip("Bloquear Horário", <LockDuoIcon className="w-4 h-4" />, false, () => setOpen("block"))}
          {chip("Editar Horário", <PenIcon className="w-4 h-4" />, false, () => setOpen("edit"))}
          {chip("Videoconferência", <VideoIcon className="w-4 h-4" />, false, () => setOpen("video"))}
          <a href={`${ROUTES.novoAgendamento}/?dia=${slot.start.slice(0, 10)}&hora=${formatTime(slot.start)}`} className="hbtn hbtn--primary hbtn--sm">
            <GridPlusIcon className="w-4 h-4" />
            <span>{rows.length ? "Encaixar Agendamento" : "Incluir Agendamento"}</span>
          </a>
          {chip("Sincronizar Google Agenda", <CalendarIcon className="w-4 h-4" />, false, () => setConfirming({ action: "sync_ga", name: agenda?.name ?? "" }))}
        </div>
                      ...
                      <tr key={row.id} className="htable-row">
                        ...
                        <td className="htable-cell">
                          <span className="inline-flex items-center gap-1">
                            {tags.join(", ") || "—"}
                            <button type="button" className="btn-icon btn-icon-sm btn-icon-flat" title="Editar Tags" onClick={() => setTags(row.id)}>
                        ...
                        <td className="htable-cell">
                          <span className="inline-flex items-center gap-1">
                            {row.comment || "—"}
                            <button type="button" className="btn-icon btn-icon-sm btn-icon-flat" title="Editar comentário" onClick={() => setComment(row.id)}>
                        ...
                        <td className="htable-cell htable-cell--center whitespace-nowrap">
                          {ACTIONS[row.status].map((action) => (
                            <button
                              ...
                              title={action.label}
                              onClick={() => setConfirming({ action: action.action, id: row.id, name: client?.name ?? "" })}
                            >
                              {action.icon}
                            </button>
                          ))}
                          <a href={`${ROUTES.novoAgendamento}/?editar=${row.id}`} className="btn-icon btn-icon-sm btn-icon-flat" title="Editar Agendamento">
                            <PenIcon className="w-4 h-4" />
                          </a>
                        </td>
      ...
      {open === "block" && <SlotBlockModal slot={slot} onClose={() => setOpen(null)} />}
      {open === "edit" && <SlotEditModal slot={slot} onClose={() => setOpen(null)} />}
